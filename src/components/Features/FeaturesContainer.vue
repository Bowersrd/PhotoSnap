...
const features = ref([
  ...
  },
  {
    ...
    icon: "features/drag-drop",
  },
]);

const visibleFeatures = props.isFeatured ? features.value.slice(0, 3) : features;
</script>

<template>
  <section class="grid px-8 py-20 text-center gap-14">
    <FeatureCard v-for="feature in visibleFeatures" :key="feature.title" :title="feature.title" :body="feature.body"
      :icon="feature.icon">
